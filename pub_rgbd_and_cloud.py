...
from utils.lib_cloud_conversion_between_Open3D_and_ROS import convertCloudFromOpen3dToRos, convertCloudFromRosToOpen3d
...
# 현재 파일의 절대 경로를 기반으로 루트 디렉토리를 설정합니다.
...
def PAB(relative_path):  # Pre-Append base_dir to the relative path.
    """
    base_dir을 상대 경로에 앞에 붙입니다.
    만약 주어진 경로가 절대 경로라면 그대로 반환합니다.
    """
...
def parse_command_line_argumetns():
    """
    명령줄 인자를 파싱합니다.
    기본값으로 설정된 base_dir과 config_file 경로를 설정합니다.
    """
